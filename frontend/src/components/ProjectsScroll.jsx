    ...
    >
      ...
      >
        <div className={`${isMobile ? '' : 'container mx-auto px-6 md:px-12 lg:px-20'} w-full`}>
          {/* Section Title */}
          <div
            className={`mb-4 md:mb-8 transform transition-all duration-1000 ease-out ${isMobile ? 'px-6' : ''} ${
          ...
          <div className="flex md:hidden justify-center gap-3 mb-4 px-6">
            {projects.map((_, idx) => (
              <button
                key={idx}
                onClick={() => handleDotClick(idx)}
              ...
              {projects.map((proj, idx) => (
                ...
                >
                  <div className="text-center py-4">
                    ...
                    <div className="mt-4">
                      <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">TECHNOLOGIES USED</div>
                      <div className="flex flex-wrap gap-2 justify-center">
                        {proj.technologies.map((tech, techIdx) => (
                          <span
                            key={techIdx}
                            className="px-2 py-1 text-gray-300 text-xs border border-white/10 rounded-lg"
                          >
                            {tech}
                          </span>
                        ))}
          ...
          {/* Desktop: Single Project Content with animation */}
          {!isMobile && (
            <>
